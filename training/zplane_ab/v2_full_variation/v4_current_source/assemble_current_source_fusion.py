"""Assemble a leak-safe v4 current-source real/complex fusion candidate.

The two input directories must be independent branches emitted by
``run_current_source_dev.py`` under one identical data, frontend, training, and
software contract.  This assembler reloads only the historical loader's
exposed train/enrollment/development-selection rows and the caller-supplied
current corpus's train/enrollment/selection roles.
...
def reject_sensitive_path(path: str | Path, role: str) -> Path:
    ...
    if (
        ...
        raise ValueError(
            "v4 development fusion refuses release, sealed, or consumed-test "
...
def validate_branch_metrics(
    ...
    *,
    ...
    if not str(run_configuration.get("schema", "")).startswith(
        "v4-current-source-development-training-"
    ):
        raise ValueError("branch run_configuration is not a v4 current-source run")
...
def load_branch(directory: str | Path, encoder: str) -> dict[str, Any]:
    """Load one v4 branch, checking every recorded artifact digest."""
...
def _source_hashes() -> dict[str, str]:
    paths = {
        "v4/assemble_current_source_fusion.py": Path(__file__).resolve(),
        "v4/run_current_source_dev.py": Path(branch_runner.__file__).resolve(),
        ...
        "v2/invariant_patch_data.py": V2 / "invariant_patch_data.py",
        "v2/run_invariant_cnn_dev.py": V2 / "run_invariant_cnn_dev.py",
...
def _expected_branch_source_hashes() -> dict[str, str]:
    """Rebuild the exact source-hash map emitted by the v4 branch runner."""
    paths = {
        "v4/run_current_source_dev.py": Path(branch_runner.__file__).resolve(),
        "v4/current_source_data.py": Path(corpus_data.__file__).resolve(),
        "v2/run_invariant_cnn_dev.py": V2 / "run_invariant_cnn_dev.py",
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if current_dir == historical_dir:
        raise ValueError("current and historical corpus directories must differ")
    ...
    if (
        ...
        raise RuntimeError(
            "branch source hashes do not reproduce from the current v4 source "
            "tree"
        )
    branch_arguments = branches["real"]["metrics"]["run_configuration"][
        "arguments"
    ]
    for field in ("current_corpus", "historical_corpus"):
        if field not in branch_arguments:
            raise ValueError(f"branch arguments are missing {field}")
        reject_sensitive_path(
            branch_arguments[field], f"recorded branch {field}"
        )
    ...
    _assert_source_snapshot_unchanged(
        ...
        operation="v4 fusion branch-source verification",
    )
    result = {
        "status": "complete",
        "development_only": True,
        ...
        "fitting_contract": {
            "center_fit_population": (
                "merged historical+current training views only"
            ),
            ...
            "prototype_fit_population": (
                "combined historical+current enrollment views only"
            ),
            ...
            "selection_population_role": (
                "metrics only; never fit, calibrated, or selected on"
            ),
    ...
    current_present_worst = final_evaluation["current"][
    ...
    print(
        ...
        f"current-present-worst={current_present_worst:.4f} "
        f"combined-pooled={combined_pooled:.4f}",
        flush=True,
    )
    print(f"[v4 fusion] wrote {output}", flush=True)
    return result
