"""Same fast-iterate + confusion-matrix harness as iterate_cnn.py, but using
train_margin.train_with_margin (prototype-repulsion loss) instead of the
plain cross-entropy trainer, to test whether explicit inter-class repulsion
stops the bluetooth/gsm/dsss crowding seen in iterate_cnn.py's vit_run1/run2.

Run:
  .venv-training/bin/python training/zplane_ab/v2_full_variation/iterate_margin.py \
      --tag margin_run1 --episodes 3000 --backbone vit
"""
...
from train_margin import train_with_margin, MARGIN, TRIAD_WEIGHT, LAMBDA_REPEL  # noqa: E402

ARTIFACTS = os.path.join(_V2_DIR, "artifacts", "iterate_margin")
...
def main():
    ...
    ap.add_argument("--triad-weight", type=float, default=TRIAD_WEIGHT)
    ap.add_argument("--lambda-repel", type=float, default=LAMBDA_REPEL)
    args = ap.parse_args()

    dev = torch.device("mps") if torch.backends.mps.is_available() else torch.device("cpu")
    print(f"[margin-{args.tag}] device={dev} episodes={args.episodes} "
          f"margin={args.margin} triad_weight={args.triad_weight} lambda_repel={args.lambda_repel}")
    ...
    print(f"[margin-{args.tag}] backbone={args.backbone} param_count={n_params}")

    eval_every = args.eval_every if args.eval_every is not None else max(100, args.episodes // 6)
    net, history, wall = train_with_margin(net, data, dev, episodes=args.episodes, eval_every=eval_every,
                                            log_tag=f"margin-{args.tag}", margin=args.margin,
                                            triad_weight=args.triad_weight, lambda_repel=args.lambda_repel)
    print(f"[margin-{args.tag}] episodes_actually_completed={history['episodes']} wall={wall:.1f}s")
    ...
    report["triad_weight"] = args.triad_weight
    report["lambda_repel"] = args.lambda_repel
    print(f"[margin-{args.tag}] closed_overall={report['closed_overall']:.3f} "
          f"closed_clean={report['closed_clean']:.3f} closed_impaired={report['closed_impaired']:.3f}")
    print(f"[margin-{args.tag}] per_class_clean={report['per_class_clean']}")
    print(f"[margin-{args.tag}] per_class_impaired={report['per_class_impaired']}")
    ...
    print(f"[margin-{args.tag}] -- CONFUSION MATRIX (OVERALL, rows=true, cols=pred) --")
    ...
    print(f"[margin-{args.tag}] done. -> {os.path.relpath(out_dir)}")
