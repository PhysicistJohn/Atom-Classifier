...
export function classify(set: PrototypeSet, embedding: Float64Array): Classification {
  ...
  return {
    label: isUnknown ? 'unknown' : set.classes[best],
    index: best,
    distanceToNearest: distances[best],
    isUnknown,
    confidence: posterior[set.classes[best]],
    posterior,
    distances,
  };
}
...
export function enroll(
  ...
  for (const e of examples) for (let k = 0; k < d; k++) proto[k] += e[k];
  let norm = 0;
  for (let k = 0; k < d; k++) {
    proto[k] /= examples.length;
    norm += proto[k] * proto[k];
  }
  norm = Math.sqrt(norm) + 1e-12;
  for (let k = 0; k < d; k++) proto[k] /= norm;
